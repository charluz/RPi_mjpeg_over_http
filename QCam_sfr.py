...
def find_houghline(img):
	if len(img.shape) > 2:
		gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
	else:
		gray = img

	lines = cv2.HoughLinesP(gray, 1, np.pi/180, 50, minLineLength=60, maxLineGap=30)
...
def find_cornerHarris(img, threshold=200, bs=2, ksize=3, Kcoef=0.04):
	if len(img.shape) > 2:
		gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
	else:
		gray = img.copy()

	#-- detecting corners --> type of element of corners is float32
	bs = 14
	Kcoef=0.01
	corners = cv2.cornerHarris(gray, bs, ksize, Kcoef)
	print("dtype of corners: ", corners.dtype)
	#-- normalize
	norms = np.empty(corners.shape, dtype=np.float32)
	cv2.normalize(corners, norms, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
	norm_scaled = cv2.convertScaleAbs(norms)
	if True:
		cv2.imwrite("corners.jpg", norm_scaled)

	#-- draw a circle at corner
	threshold = 220
	print("num of corners: ", (norms > threshold).sum())
	print(norms)
	for y in range(norms.shape[0]):
		for x in range(norms.shape[1]):
			if int(norms[y, x]) > threshold:
				cv2.circle(img, (x, y), 5, (0, 255, 255), 6)
...
def focusing_scoring(frame, roi_rects):
	...
	for vtx in vertices:
		...
		# 找 lines、 corners
		#----------------------------------------
		# find_houghline(lap_img)
		find_cornerHarris(lap_img)
		...
		#-- 鑲嵌 edge 影像
		implant_frame_roi(frame, roi_img, Vt, Vb)
